...
mbx_dr.put(drv_trans);
           repeat(1) @(vif.drv_cb);
           $display("DRIVER DRIVING DATA TO THE INTERFACE data_in=%d,write_enb=%d,read_enb=%d,address=%d,reset=%d",vif.drv_cb.data_in,vif.drv_cb.write_enb,vif.drv_cb.read_enb,vif.drv_cb.address,vif.drv_cb.reset,$time);
          end
        else
         repeat(1) @(vif.drv_cb)
          begin
               ...
               repeat(1) @(vif.drv_cb);
               $display("DRIVER WRITE OPERATION DRIVING DATA TO THE INTERFACE data_in=%d,write_enb=%d,read_enb=%d,address=%d,reset=%d",vif.drv_cb.data_in,vif.drv_cb.write_enb,vif.drv_cb.read_enb,vif.drv_cb.address,vif.drv_cb.reset,$time);
               vif.drv_cb.write_enb<=0;
