...
//METHODS
  ...
  task start();
blueprint.write_enb=1;
    blueprint.read_enb=0;
    blueprint.address=5;
    blueprint.data_in =8'h11;
    mbx_gd.put(blueprint.copy());

    // Directed read
    blueprint.write_enb=0;
    blueprint.read_enb=1;
    blueprint.address=5;
    mbx_gd.put(blueprint.copy());
    for(int i=0;i<`no_of_trans;i++)
      ...
      //Putting the randomized inputs to mailbox
        mbx_gd.put(blueprint.copy());
        $display("GENERATOR Randomized transaction data_in=%d,write_enb=%d,read_enb=%d,address=%d",blueprint.data_in,blueprint.write_enb,blueprint.read_enb,blueprint.address,$time);
      end
  endtask
endclass
